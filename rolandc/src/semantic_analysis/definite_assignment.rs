...
pub fn ensure_variables_definitely_assigned(program: &Program, err_manager: &mut ErrorManager) {
   for procedure in program.procedures.values() {
      if let ProcImplSource::Body(block) = &procedure.proc_impl {
         ...
         ensure_all_variables_assigned_in_block(block, &mut unassigned_vars, &program.ast, err_manager);
      }
   }
}
...
fn ensure_all_variables_assigned_in_block(
   ...
   for stmt_id in proc_body.statements.iter().copied() {
      ensure_all_variables_assigned_in_stmt(stmt_id, unassigned_vars, pool, err_manager);
   }
}
...
fn ensure_all_variables_assigned_in_stmt(
   ...
   match &sn.statement {
      ...
      Statement::Block(b) => {
         ensure_all_variables_assigned_in_block(&b, unassigned_vars, pool, err_manager);
      }
      Statement::IfElse(cond, then, otherwise) => {
         ...
         ensure_all_variables_assigned_in_block(&then, unassigned_vars, pool, err_manager);
         ensure_all_variables_assigned_in_stmt(*otherwise, &mut else_unassigned, pool, err_manager);
   ...
   match &en.expression {
      Expression::Variable(var_id) => {
         if unassigned_vars.contains(var_id) {
            rolandc_error!(err_manager, en.location, "Variable may be unassigned before use");
