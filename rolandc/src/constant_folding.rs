...
fn fold_expr(
   ...
   match unsafe { &mut (*expr_to_fold).expression } {
      Expression::ArrayIndex { array, index } => {
         ...
         let Some(ValueType::Array(_, len)) = array.exp_type else { unreachable!() };
            ...
            match operator {
               ...
               BinOp::BitwiseXor => {
                  let expr = match folding_context.expressions[expr_index].exp_type.as_ref() {
                     Some(ValueType::Bool) => Expression::BoolLiteral(false),
                     Some(ValueType::Int { .. }) => Expression::IntLiteral {
                        val: 0,
                        synthetic: true,
                     },
                     _ => unreachable!(),
...
impl Literal {
   ...
   fn transmute(self, target_type: &ExpressionType) -> Option<Expression> {
      Some(match (self, target_type) {
         ...
         (Literal::Float32(f), &ValueType::Int(_)) => Expression::IntLiteral {
            val: u64::from(f.to_bits()),
            synthetic: true,
         },
         (Literal::Float64(f), &ValueType::Int(_)) => Expression::IntLiteral {
            val: f.to_bits(),
            synthetic: true,
         },
         ...
         (Literal::Int64(i), &ValueType::Int(it)) if !it.signed => Expression::IntLiteral {
            val: i as u64,
            synthetic: true,
         },
         (Literal::Int32(i), &ValueType::Int(it)) if !it.signed => Expression::IntLiteral {
            val: u64::from(i as u32),
            synthetic: true,
         },
         (Literal::Int16(i), &ValueType::Int(it)) if !it.signed => Expression::IntLiteral {
            val: u64::from(i as u16),
            synthetic: true,
         },
         (Literal::Int8(i), &ValueType::Int(it)) if !it.signed => Expression::IntLiteral {
            val: u64::from(i as u8),
            synthetic: true,
         },

         // Transmute unsigned -> signed
         (Literal::Uint64(i), &ValueType::Int(it)) if it.signed => Expression::IntLiteral {
            val: i,
            synthetic: true,
         },
         (Literal::Uint32(i), &ValueType::Int(it)) if it.signed => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },
         (Literal::Uint16(i), &ValueType::Int(it)) if it.signed => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },
         (Literal::Uint8(i), &ValueType::Int(it)) if it.signed => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },

         // Noop
         (Literal::Int64(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i as u64,
            synthetic: true,
         },
         (Literal::Int32(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i64::from(i) as u64,
            synthetic: true,
         },
         (Literal::Int16(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i64::from(i) as u64,
            synthetic: true,
         },
         (Literal::Int8(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i64::from(i) as u64,
            synthetic: true,
         },
         (Literal::Uint64(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i,
            synthetic: true,
         },
         (Literal::Uint32(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },
         (Literal::Uint16(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },
         (Literal::Uint8(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: u64::from(i),
            synthetic: true,
         },
         _ => return None,
   ...
   fn extend(self, target_type: &ExpressionType) -> Option<Expression> {
      Some(match (self, target_type) {
         (Literal::Float64(f), &ValueType::Float(_)) => Expression::FloatLiteral(f),
         (Literal::Float32(f), &ValueType::Float(_)) => Expression::FloatLiteral(f64::from(f)),
         (Literal::Int64(i), &ValueType::Int(_)) => Expression::IntLiteral {
            val: i as u64,
            synthetic: true,
         },
