...
fn vv_expr(
   ...
   match &expressions[expr_index].expression {
      ...
      Expression::ProcedureCall { args, proc_expr } => {
         ...
         let exp_type = expressions[expr_index]
         .exp_type
         .as_ref()
         .unwrap();
         ...
         if (exp_type.size_is_unknown(vv_context.user_defined_type_info) || exp_type.is_or_contains_union(vv_context.user_defined_type_info))
            && !top
         {
